...
parser.add_argument("--sweep-dx", type=float, default=0.1)
parser.add_argument("--sweep-dy", type=float, default=0.1)
parser.add_argument("--dwell-frames", type=int, default=120)
...
def create_marker(stage, prim_path, radius=0.02, color=(0.85, 0.18, 0.16)):
...
current_position, current_rotation = art_kinematics.compute_end_effector_pose()
...
corner_offsets = [
    np.array([-args.sweep_dx, args.sweep_dy, 0.0], dtype=np.float64),
    np.array([args.sweep_dx, args.sweep_dy, 0.0], dtype=np.float64),
    np.array([args.sweep_dx, -args.sweep_dy, 0.0], dtype=np.float64),
    np.array([-args.sweep_dx, -args.sweep_dy, 0.0], dtype=np.float64),
]
corner_targets = [np.array([args.target_x, args.target_y, args.target_z], dtype=np.float64) + offset for offset in corner_offsets]

target_marker = create_marker(stage, "/World/Debug/SquareTarget", radius=0.02, color=(0.88, 0.18, 0.16))

corner_index = 0
corner_hold = 0
last_printed_corner = None

print(f"[PiperSquare] base target position: {[args.target_x, args.target_y, args.target_z]}")
print(f"[PiperSquare] orientation offset deg: {[args.roll, args.pitch, args.yaw]}")
print(f"[PiperSquare] square corners: {[corner.tolist() for corner in corner_targets]}")

while simulation_app.is_running():
    target_position = corner_targets[corner_index]
    ...
    elif last_printed_corner != corner_index:
        print(f"[PiperSquare] IK failed at corner {corner_index}: {target_position.tolist()}")

    if last_printed_corner != corner_index:
        print(f"[PiperSquare] tracking corner {corner_index}: {target_position.tolist()}")
        last_printed_corner = corner_index

    world.step(render=True)

    current_position, _ = art_kinematics.compute_end_effector_pose()
    if np.linalg.norm(current_position - target_position) < 0.03:
        corner_hold += 1
        if corner_hold >= args.dwell_frames:
            corner_index = (corner_index + 1) % len(corner_targets)
            corner_hold = 0
            last_printed_corner = None
    else:
        corner_hold = 0
