...
DEFAULT_DATA_DIR = os.path.join(REPO_ROOT, "data", "clutter_pick_target_raw")
...
STATIC_CAMERA_EYE = [-0.248822, 0.555326, 0.355019]
STATIC_CAMERA_TARGET = [-0.248822, -0.233189, -0.259997]
STATIC_CAMERA_FOCAL_LENGTH = 1.93
STATIC_CAMERA_HORIZONTAL_APERTURE = 3.728
STATIC_CAMERA_VERTICAL_APERTURE = 2.799
STATIC_CAMERA_CLIPPING_RANGE = (0.01, 100.0)
...
def create_topdown_camera(stage, prim_path, center, coverage_xy):
    ...
    set_camera_view(
        eye=STATIC_CAMERA_EYE,
        target=STATIC_CAMERA_TARGET,
        camera_prim_path=prim_path,
    )
    camera.CreateFocalLengthAttr(STATIC_CAMERA_FOCAL_LENGTH)
    camera.CreateHorizontalApertureAttr(STATIC_CAMERA_HORIZONTAL_APERTURE)
    camera.CreateVerticalApertureAttr(STATIC_CAMERA_VERTICAL_APERTURE)
    camera.CreateClippingRangeAttr(Gf.Vec2f(*STATIC_CAMERA_CLIPPING_RANGE))
    return camera.GetPrim()
...
def generate_spawned_entries(seed, source_center, tray_width, tray_depth, tray_wall, drop_base_z, object_specs):
    ...
    spawned = []
    random_specs = [spec for spec in object_specs if not bool(spec.get("is_target", False))]
    target_specs = [spec for spec in object_specs if bool(spec.get("is_target", False))]
    ...
    for spec in target_specs:
        ...
        spawned.append(
            {
                **spec,
                "initial_position": [target_x, target_y, float(drop_base_z + TARGET_DROP_HEIGHT_OFFSET_Z)],
                "initial_orientation_wxyz": quat.tolist(),
            }
        )
    return spawned
...
scene_config["scene_name"] = "clutter_pick_target"
scene_config["task"]["name"] = "clutter_pick_target_marker"
...
topdown_camera_prim_path = "/World/Debug/TopDownCamera"
...
if not random_asset_pool:
    raise RuntimeError(f"Target asset {TARGET_USD_NAME} exhausted the YCB pool")
...
for local_idx, asset_idx in enumerate(selected_indices):
    ...
    target_x = float(source_tray_center[0] + inner_half_x - inner_half_x * TARGET_CORNER_INSET_X_FRACTION)
    target_y = float(source_tray_center[1] - inner_half_y + inner_half_y * TARGET_CORNER_INSET_Y_FRACTION)
    for _ in range(128):
        candidate_x = float(source_tray_center[0] + rng.uniform(-inner_half_x, inner_half_x))
        candidate_y = float(source_tray_center[1] + rng.uniform(-inner_half_y, inner_half_y))
        inside_keepout = (
            abs(candidate_x - target_x) <= TARGET_KEEP_OUT_HALF_EXTENT_X
            and abs(candidate_y - target_y) <= TARGET_KEEP_OUT_HALF_EXTENT_Y
        )
        ...
        x = float(source_tray_center[0] - inner_half_x + 0.02)
        y = float(source_tray_center[1] + inner_half_y - 0.02)
    z = float(drop_base_z + 0.06 * local_idx)
    yaw = float(rng.uniform(0.0, 360.0))
    quat = np.array([np.cos(np.deg2rad(yaw) * 0.5), 0.0, 0.0, np.sin(np.deg2rad(yaw) * 0.5)], dtype=np.float32)
...
target_idx = len(clutter_objects)
target_prim_path = f"/World/Clutter/Object_{target_idx}"
add_reference_to_stage(usd_path=target_usd_path, prim_path=target_prim_path)
auto_generate_convex_colliders(stage.GetPrimAtPath(target_prim_path))
target_x = float(source_tray_center[0] + inner_half_x - inner_half_x * TARGET_CORNER_INSET_X_FRACTION)
target_y = float(source_tray_center[1] - inner_half_y + inner_half_y * TARGET_CORNER_INSET_Y_FRACTION)
target_z = float(drop_base_z + TARGET_DROP_HEIGHT_OFFSET_Z)
target_yaw = float(rng.uniform(0.0, 360.0))
target_quat = np.array(
    [np.cos(np.deg2rad(target_yaw) * 0.5), 0.0, 0.0, np.sin(np.deg2rad(target_yaw) * 0.5)],
    dtype=np.float32,
)
target_rigid = world.scene.add(
    SingleRigidPrim(
        prim_path=target_prim_path,
        name=f"clutter_object_{target_idx}",
...
target_bbox_size_np = np.array([float(target_bbox_size[0]), float(target_bbox_size[1]), float(target_bbox_size[2])], dtype=np.float32)
clutter_objects.append(
    {
        "rigid": target_rigid,
        "name": os.path.basename(target_usd_path),
        "bbox_size": target_bbox_size_np,
        "initial_position": np.array([target_x, target_y, target_z], dtype=np.float32),
        "initial_orientation": target_quat.copy(),
        "is_target": True,
    }
)
object_info.append(
    {
        ...
        "initial_orientation_wxyz": target_quat.tolist(),
        "bbox_size": target_bbox_size_np.tolist(),
        "is_target": True,
    }
)
...
if args.camera_panels:
    topdown_window = build_camera_window("Clutter Pick Target Top Down", topdown_provider, 640, 480, 980, 40)
    robot_window = build_camera_window("Clutter Pick Target Wrist", robot_provider, 640, 480, 980, 560)
...
world.reset()
topdown_sensor_camera = world.scene.add(
    Camera(
        prim_path=topdown_camera_prim_path,
        name="topdown_sensor_camera",
        frequency=30,
        resolution=(640, 480),
    )
)
set_camera_view(eye=STATIC_CAMERA_EYE, target=STATIC_CAMERA_TARGET, camera_prim_path=topdown_camera_prim_path)
world.reset()
...
topdown_sensor_camera.set_focal_length(STATIC_CAMERA_FOCAL_LENGTH)
topdown_sensor_camera.set_horizontal_aperture(STATIC_CAMERA_HORIZONTAL_APERTURE)
topdown_sensor_camera.set_vertical_aperture(STATIC_CAMERA_VERTICAL_APERTURE)
topdown_sensor_camera.set_clipping_range(*STATIC_CAMERA_CLIPPING_RANGE)
topdown_sensor_camera.resume()
...
print(f"[ClutterPickTarget] source tray center: {source_tray_center}")
print(f"[ClutterPickTarget] target tray center: {target_tray_center}")
print(f"[ClutterPickTarget] ycb bank size after volume filter < {args.max_volume}: {len(ycb_assets)}")
print(f"[ClutterPickTarget] spawned objects: {len(clutter_objects)}")
print(f"[ClutterPickTarget] Lula active joints: {lula_solver.get_joint_names()}")
print(f"[ClutterPickTarget] camera panels: {'enabled' if args.camera_panels else 'disabled'}")
if args.camera_panels:
    print(f"[ClutterPickTarget] top-down camera panel: {topdown_window.title}")
    print(f"[ClutterPickTarget] wrist camera panel: {robot_window.title}")
for entry in clutter_objects:
    target_tag = " [target]" if entry.get("is_target", False) else ""
    print(f"  - {entry['name']}{target_tag}")
...
try:
    while simulation_app.is_running():
            ...
            if not recording:
                ...
                save_futures.append(
                    save_executor.submit(
                        ...
                        episode=episode,
                        task_name="clutter_pick_target_marker",
        ...
        if 1 in buttons and 1 not in last_buttons:
            ...
            current_seed += 1
            respawned = generate_spawned_entries(
                seed=current_seed,
                source_center=source_tray_center,
                tray_width=args.tray_width,
                tray_depth=args.tray_depth,
                tray_wall=args.tray_wall,
                drop_base_z=drop_base_z,
                object_specs=base_object_specs,
            )
            for entry, respawn_info, item in zip(clutter_objects, respawned, object_info):
                position = np.asarray(respawn_info["initial_position"], dtype=np.float32)
                orientation = np.asarray(respawn_info["initial_orientation_wxyz"], dtype=np.float32)
                ...
                entry["initial_position"] = position.copy()
                entry["initial_orientation"] = orientation.copy()
                item["initial_position"] = position.tolist()
                item["initial_orientation_wxyz"] = orientation.tolist()
            scene_config["objects"]["spawned"] = copy.deepcopy(object_info)
            print(f"[PS5] target reset -> new clutter seed {current_seed}")
