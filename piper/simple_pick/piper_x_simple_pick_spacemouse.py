...
class SpaceMouseReader:
    def __init__(self, vendor_id: str, product_id: str):
        ...
        self.class_name = "IsaacSimplePickSpaceMouseWindow"
        self.device_names = {}
        self.running = False
        self.lock = threading.Lock()
        self.state = {"translation": (0, 0, 0), "rotation": (0, 0, 0), "buttons": 0, "updated_at": time.time()}
    ...
    def _register_raw_input(self):
        raw_input_device = RAWINPUTDEVICE(usUsagePage=TARGET_USAGE_PAGE, usUsage=TARGET_USAGE, dwFlags=RIDEV_INPUTSINK, hwndTarget=self.hwnd)
...
def build_status_window(initial_position):
    ...
    with window.frame:
        with ui.VStack(spacing=8, height=0):
            ui.Label("SpaceMouse teleop for the simple-pick scene.")
            ui.Label("Button 1: gripper toggle | Button 2: reset target")
...
parser.add_argument("--translation-scale", type=float, default=0.000035)
...
while simulation_app.is_running():
    ...
    delta = np.array([-trans[0] * args.translation_scale, trans[1] * args.translation_scale, -trans[2] * args.translation_scale], dtype=np.float32)
    target_position += delta
    rot_delta = np.array([rot[0] * args.rotation_scale, rot[1] * args.rotation_scale, rot[2] * args.rotation_scale], dtype=np.float64)
