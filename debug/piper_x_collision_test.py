...
def build_ik_window(initial_position):
    ...
    window = ui.Window("Collision Lula IK", width=380, height=240, visible=True)
...
print(f"[CollisionTest] articulation prim: /World/RobotMount/PiperX/Robot")
print(f"[CollisionTest] dof count: {len(joint_names)}")
print(f"[CollisionTest] joints: {joint_names}")
for gripper_joint in ("joint7", "joint8"):
    idx = joint_name_to_index[gripper_joint]
    print(
        f"[CollisionTest] {gripper_joint} props: "
...
print("[CollisionTest] applied gripper hold override on joint7/8")
...
print(f"[CollisionTest] Lula YAML: {os.path.abspath(args.lula_yaml)}")
print(f"[CollisionTest] Lula URDF: {os.path.abspath(args.lula_urdf)}")
print(f"[CollisionTest] EE frame: {args.ee_frame}")
print(f"[CollisionTest] current ee position: {ee_position}")
print(f"[CollisionTest] Lula active joints: {lula_solver.get_joint_names()}")
